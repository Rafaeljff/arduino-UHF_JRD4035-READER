...
void Menu_main() {
	...
	int http_menu = http_get_message();
	int selection = 0;
	...
	if (user_menu == 1 || http_menu == 1)
		selection = 1;
	else if (user_menu == 2 || http_menu == 2)
		selection = 2;
	else if (user_menu == 3 || http_menu == 3)
		selection = 3;
	else if (user_menu == 4 || http_menu == 4)
		selection = 4;
	else
		selection = 0;
...
//				count2++;
			...
			if (myFile) {
				...
				myFile.close();
				//Serial.print("\nUKLTIMOOOOOOOOOOOOOOOOOOOOO CHAAAAAAAAAAAAAAARRRRRRRRRR CRALAHO");
	...
	if (WiFi.status() == WL_CONNECTED) {   //Check WiFi connection status
		...
		if (httpResponseCode > 0) {
			String response = http.getString(); //Get the response to the request
				...
				Serial.println("\nbutton from node red pressed!!");
				Serial.println("Menu selected from node_red: ");
